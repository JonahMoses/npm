...
var statsFile = path.join(cachedir, 'anonymous-cli-stats.json')
  ...
  testdir: Dir({
    ...
    slow: Dir({
      'package.json': File({
        ...
        version: '1.0.0',
        scripts: {
          preinstall: "node -e 'setTimeout(function(){}, 1500)'"
        }
      })
    }),
    'package.json': File({
...
test('record success', function (t) {
  common.npm(['install', '--no-send-metrics', 'success'], conf, function (err, code, stdout, stderr) {
    ...
    t.is(stats.metrics.successfulInstalls, 1)
    t.is(stats.metrics.failedInstalls, 0)
    t.done()
  })
})
...
test('record failure', function (t) {
  ...
  common.npm(['install', '--send-metrics', 'failure'], conf, function (err, code, stdout, stderr) {
    ...
    var stats = JSON.parse(fs.readFileSync(statsFile))
    t.is(stats.metrics.successfulInstalls, 1)
    t.is(stats.metrics.failedInstalls, 1)
    t.done()
  })
})
